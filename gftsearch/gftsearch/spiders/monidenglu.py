...
class SsrnSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        return [scrapy.Request(url='https://hq.ssrn.com/rankings/Ranking_display.cfm?TRN_gID=10', cookies=cookies)]

    def parse(self, response):
        for i in response.xpath('//*[@id="maincontent"]/div[6]/table/tbody/tr'):
            yield {
                "gid": i.xpath('./@id').extract_first(),
                "links": i.xpath('./td/a[@class="paper-title"]/@href').extract_first(),
                "title": i.xpath('./td/a/text()').extract_first(),
                "author": i.xpath('./td/span/a/text()').extract_first(),
                "downloads": i.xpath('./td[@class="shown-2 selected"]/a/text()').extract_first().strip(),
            }
